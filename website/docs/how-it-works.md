...
Scroblarr turns **scrobble** events into watched state for **movies** and **TV episodes**, using the metadata your media server sends (titles, IDs, season/episode, etc.). Destination-specific behavior (progress, TVTime rewatch flags, TVDB expectations) is documented under **[Trakt](/docs/configuration/trakt#what-gets-synced)** and **[TVTime](/docs/configuration/tvtime#what-gets-synced)**.
