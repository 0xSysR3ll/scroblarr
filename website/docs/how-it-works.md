...
4. **Scroblarr syncs to your linked accounts** (Trakt, TVTime, Simkl) automatically
...
- **Each user on your media server** can link their own Trakt, TVTime, and Simkl accounts
...
Scroblarr turns **scrobble** events into watched state for **movies** and **TV episodes**, using the metadata your media server sends (titles, IDs, season/episode, etc.). Destination-specific behavior (progress, TVTime rewatch flags, TVDB expectations, Simkl matching) is documented under **[Trakt](/docs/configuration/trakt#what-gets-synced)**, **[TVTime](/docs/configuration/tvtime#what-gets-synced)**, and **[Simkl](/docs/configuration/simkl#what-gets-synced)**.
