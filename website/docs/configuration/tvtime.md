...
## Metadata requirements (TVDB)

TVTime's API expects stable **TheTVDB** identifiers when possible:

- **Episodes**: The webhook payload must include a **TVDB episode ID**. If Plex or Jellyfin does not expose TVDB-style GUIDs in metadata, episode sync to TVTime can fail with errors mentioning a missing TVDB episode ID. Prefer agents / metadata sources that provide TVDB IDs, and refresh metadata for the show in your media server if needed.
- **Movies**: TVTime can fall back to **IMDb** or title search when TVDB movie IDs are missing, but matches are more reliable when TVDB (or IMDb) IDs are present.

This is a limitation of how TVTime identifies titles, not something you configure inside Scroblarr.
